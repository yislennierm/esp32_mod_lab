...
#define FRAME_COUNT 240U
#define TARGET_FPS 59.73

static const char *TAG = "ppa_srm_bench";
...
static void fill_source(uint16_t *src)
{
    for (uint32_t y = 0; y < SRC_H; ++y) {
        for (uint32_t x = 0; x < SRC_W; ++x) {
            const uint8_t r = (uint8_t)((x * 255U) / (SRC_W - 1U));
            const uint8_t g = (uint8_t)((y * 255U) / (SRC_H - 1U));
...
static int64_t bench_cpu_scale2x(const uint16_t *src, uint16_t *dst, uint32_t frames)
{
    ...
    for (uint32_t frame = 0; frame < frames; ++frame) {
        for (uint32_t y = 0; y < SRC_H; ++y) {
            const uint16_t *in = src + (y * SRC_W);
            uint16_t *out0 = dst + ((y * 2U) * DST_W);
            uint16_t *out1 = out0 + DST_W;
            for (uint32_t x = 0; x < SRC_W; ++x) {
                         ...
                         uint32_t checksum)
{
    ...
    const double target_frame_us = 1000000.0 / TARGET_FPS;
    const bool target_rate_met = fps >= (TARGET_FPS * 0.98);
    printf("{\"ok\":%s,\"command\":\"%s\",\"schema\":\"esp32_mod_lab.benchmark.ppa_srm.v1\","
           "\"mode\":\"synthetic_rgb565_scale2x\","
...
void app_main(void)
{
    ESP_LOGI(TAG, "PPA SRM benchmark; no source GPIO, USB stream, browser, or SPI LCD hot path");

    const size_t src_bytes = SRC_W * SRC_H * BYTES_PER_PIXEL;
    const size_t dst_bytes = DST_W * DST_H * BYTES_PER_PIXEL;
    size_t alignment = 0;
    ...
    while (true) {
        memset(ppa_dst, 0, dst_bytes);
        int64_t ppa_elapsed_us = 0;
        err = bench_ppa_scale2x(client, src, ppa_dst, src_bytes, dst_bytes, FRAME_COUNT, &ppa_elapsed_us);
        const uint32_t ppa_checksum = checksum_sample(ppa_dst, DST_W * DST_H);
        print_result("PPA_SRM_SCALE2X_BENCH", err, FRAME_COUNT, src_bytes, dst_bytes, ppa_elapsed_us, ppa_checksum);

        memset(cpu_dst, 0, dst_bytes);
        const int64_t cpu_elapsed_us = bench_cpu_scale2x(src, cpu_dst, FRAME_COUNT);
        const uint32_t cpu_checksum = checksum_sample(cpu_dst, DST_W * DST_H);
        print_result("CPU_SCALE2X_BENCH", ESP_OK, FRAME_COUNT, src_bytes, dst_bytes, cpu_elapsed_us, cpu_checksum);
