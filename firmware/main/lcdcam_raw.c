...
static bool s_loop_summary_enabled = true;
...
#define LCDCAM_RAW_CTLR_BUFFER_SLOTS 3U
#define LCDCAM_RAW_RING_BUFFER_SLOTS 3U

typedef struct {
    uint8_t *buffers[LCDCAM_RAW_CTLR_BUFFER_SLOTS];
    size_t buffer_len;
    volatile uint32_t next_slot;
    volatile uint32_t completed_frames;
    volatile uint32_t dropped_frames;
    volatile uint32_t last_finished_slot;
    uint32_t requested_frames;
    volatile int64_t started_us;
    int64_t *frame_done_us;
    SemaphoreHandle_t done_sem;
} lcdcam_raw_ctlr_context_t;
...
typedef struct {
    ...
    lcdcam_raw_dma_desc_t *desc;
    uint8_t **buffers;
    size_t desc_count_per_slot;
    size_t desc_stride_per_slot;
    size_t desc_size_per_slot;
    size_t buffer_len;
    uint32_t requested_frames;
    volatile uint32_t completed_frames;
    volatile uint32_t ring_rearms;
    volatile uint32_t ring_rearm_failures;
    volatile uint32_t unknown_eof_desc;
    volatile int64_t started_us;
    int64_t *frame_done_us;
    esp_err_t last_esp_err;
} lcdcam_raw_ring_context_t;

typedef struct {
    gdma_channel_handle_t dma_chan;
    lcd_cam_dev_t *hw;
    QueueHandle_t completed_slots;
    lcdcam_raw_dma_desc_t *desc;
    size_t desc_count_per_slot;
    size_t desc_stride_per_slot;
    size_t buffer_len;
    size_t buffer_msync_len;
    uint32_t requested_frames;
    volatile uint32_t completed_frames;
    volatile uint32_t unknown_eof_desc;
    volatile int64_t started_us;
    int64_t *frame_done_us;
} lcdcam_raw_ring_stream_context_t;
...
static void fill_ring_descriptors(lcdcam_raw_dma_desc_t *desc,
                                  uint8_t **buffers,
                                  size_t slot_count,
                                  size_t desc_count_per_slot,
                                  size_t desc_stride_per_slot,
                                  size_t buffer_len)
{
    for (size_t slot = 0; slot < slot_count; ++slot) {
        lcdcam_raw_dma_desc_t *slot_base = &desc[slot * desc_stride_per_slot];
        fill_descriptors(slot_base, buffers[slot], buffer_len);
        size_t next_slot = (slot + 1U) % slot_count;
        slot_base[desc_count_per_slot - 1U].next = &desc[next_slot * desc_stride_per_slot];
    }
}

static void IRAM_ATTR restore_ring_descriptor_links(lcdcam_raw_dma_desc_t *desc,
                                                    size_t slot_count,
                                                    size_t desc_count_per_slot,
                                                    size_t desc_stride_per_slot)
{
    for (size_t slot = 0; slot < slot_count; ++slot) {
        lcdcam_raw_dma_desc_t *slot_base = &desc[slot * desc_stride_per_slot];
        for (size_t i = 0; i + 1U < desc_count_per_slot; ++i) {
            slot_base[i].next = &slot_base[i + 1U];
        }
        size_t next_slot = (slot + 1U) % slot_count;
        slot_base[desc_count_per_slot - 1U].next = &desc[next_slot * desc_stride_per_slot];
    }
...
static bool wait_for_start_trigger(lcdcam_raw_start_mode_t start_mode, uint32_t timeout_ms)
{
    ...
    case LCDCAM_RAW_START_AFTER_SPS_RISING:
        return wait_for_edge(GBC_LCD_GPIO_SPS, 0, 1, timeout_ms);
    case LCDCAM_RAW_START_AFTER_SPS_FALLING:
        return wait_for_edge(GBC_LCD_GPIO_SPS, 1, 0, timeout_ms);
    case LCDCAM_RAW_START_AFTER_SPS_THEN_SPL_FALLING:
        if (!wait_for_edge(GBC_LCD_GPIO_SPS, 0, 1, timeout_ms)) {
            return false;
        }
        return wait_for_edge(GBC_LCD_GPIO_SPL, 1, 0, timeout_ms);
...
static bool IRAM_ATTR on_ctlr_get_new_trans(esp_cam_ctlr_handle_t handle,
                                            esp_cam_ctlr_trans_t *trans,
                                            void *user_data)
{
    (void)handle;
    lcdcam_raw_ctlr_context_t *context = (lcdcam_raw_ctlr_context_t *)user_data;
    uint32_t slot = context->next_slot % LCDCAM_RAW_CTLR_BUFFER_SLOTS;
    context->next_slot = (slot + 1U) % LCDCAM_RAW_CTLR_BUFFER_SLOTS;
    trans->buffer = context->buffers[slot];
    trans->buflen = context->buffer_len;
    return true;
}

static bool IRAM_ATTR on_ctlr_trans_finished(esp_cam_ctlr_handle_t handle,
                                             esp_cam_ctlr_trans_t *trans,
                                             void *user_data)
{
    (void)handle;
    BaseType_t high_task_woken = pdFALSE;
    lcdcam_raw_ctlr_context_t *context = (lcdcam_raw_ctlr_context_t *)user_data;
    uint32_t completed = context->completed_frames;
    if (completed < context->requested_frames) {
        context->frame_done_us[completed] = esp_timer_get_time() - context->started_us;
    }
    context->completed_frames = completed + 1U;
    context->last_finished_slot = UINT32_MAX;
    for (uint32_t i = 0; i < LCDCAM_RAW_CTLR_BUFFER_SLOTS; ++i) {
        if (context->buffers[i] == trans->buffer) {
            context->last_finished_slot = i;
            break;
        }
    }
    xSemaphoreGiveFromISR(context->done_sem, &high_task_woken);
...
static void IRAM_ATTR rearm_descriptor_slot(lcdcam_raw_dma_desc_t *desc,
                                            size_t desc_count,
                                            uint8_t *buffer,
                                            size_t size)
{
    size_t index = 0;
    while (size > 0 && index < desc_count) {
        uint32_t node_size = MIN(size, LCDCAM_RAW_DMA_DESC_MAX_SIZE);
        desc[index].dw0.size = node_size;
        desc[index].dw0.length = 0;
        desc[index].dw0.err_eof = 0;
...
static bool IRAM_ATTR on_ring_recv_eof(gdma_channel_handle_t dma_chan,
                                       gdma_event_data_t *event_data,
                                       void *user_data)
{
    (void)dma_chan;
    BaseType_t high_task_woken = pdFALSE;
    lcdcam_raw_ring_context_t *context = (lcdcam_raw_ring_context_t *)user_data;
    intptr_t eof_desc_addr = event_data == NULL ? 0 : event_data->rx_eof_desc_addr;
    uint32_t slot = UINT32_MAX;
    for (uint32_t i = 0; i < LCDCAM_RAW_RING_BUFFER_SLOTS; ++i) {
        lcdcam_raw_dma_desc_t *slot_base = &context->desc[i * context->desc_stride_per_slot];
        intptr_t first = (intptr_t)slot_base;
        intptr_t last = (intptr_t)&slot_base[context->desc_count_per_slot - 1U];
        if (eof_desc_addr >= first && eof_desc_addr <= last) {
            slot = i;
            break;
        }
    }
    ...
    if (slot == UINT32_MAX) {
        context->unknown_eof_desc++;
    } else if (context->completed_frames < context->requested_frames) {
        lcdcam_raw_dma_desc_t *slot_base = &context->desc[slot * context->desc_stride_per_slot];
        rearm_descriptor_slot(slot_base, context->desc_count_per_slot, context->buffers[slot], context->buffer_len);
        restore_ring_descriptor_links(context->desc,
                                      LCDCAM_RAW_RING_BUFFER_SLOTS,
                                      context->desc_count_per_slot,
                                      context->desc_stride_per_slot);
        esp_err_t err = esp_cache_msync(context->desc,
                                        context->desc_size_per_slot * LCDCAM_RAW_RING_BUFFER_SLOTS,
                                        ESP_CACHE_MSYNC_FLAG_DIR_C2M | ESP_CACHE_MSYNC_FLAG_INVALIDATE);
        if (err == ESP_OK) {
            context->ring_rearms++;
        } else {
            context->last_esp_err = err;
            context->ring_rearm_failures++;
        }
    }

    if (context->completed_frames >= context->requested_frames || context->ring_rearm_failures > 0) {
        cam_ll_stop(context->hw);
        (void)gdma_stop(context->dma_chan);
        xSemaphoreGiveFromISR(context->done_sem, &high_task_woken);
    }
...
static bool IRAM_ATTR on_ring_stream_recv_eof(gdma_channel_handle_t dma_chan,
                                              gdma_event_data_t *event_data,
                                              void *user_data)
{
    (void)dma_chan;
    BaseType_t high_task_woken = pdFALSE;
    lcdcam_raw_ring_stream_context_t *context = (lcdcam_raw_ring_stream_context_t *)user_data;
    intptr_t eof_desc_addr = event_data == NULL ? 0 : event_data->rx_eof_desc_addr;
    uint32_t slot = UINT32_MAX;
    for (uint32_t i = 0; i < LCDCAM_RAW_RING_BUFFER_SLOTS; ++i) {
        lcdcam_raw_dma_desc_t *slot_base = &context->desc[i * context->desc_stride_per_slot];
    ...
    if (slot == UINT32_MAX) {
        ...
        (void)xQueueSendFromISR(context->completed_slots, &slot, &high_task_woken);
    }

    if (context->completed_frames >= context->requested_frames || context->unknown_eof_desc > 0) {
        cam_ll_stop(context->hw);
        (void)gdma_stop(context->dma_chan);
    }
    return high_task_woken == pdTRUE;
...
{
    s_loop_summary_enabled = enabled;
}
                                     ...
                                     lcdcam_raw_data_mode_t data_mode)
{
    ...
    ESP_RETURN_ON_ERROR(configure_input_gpio(GBC_LCD_GPIO_SPS, cam_periph_signals.buses[0].vsync_sig, vsync_invert),
    ...
    ESP_RETURN_ON_ERROR(configure_input_gpio(GBC_LCD_GPIO_DCLK, cam_periph_signals.buses[0].pclk_sig, pclk_invert),
                        "lcdcam_raw", "pclk route");

    const int rg44_data_gpio[8] = {
        GBC_LCD_GPIO_R2, /* bit0: R2 */
        GBC_LCD_GPIO_R3, /* bit1: R3 */
        GBC_LCD_GPIO_R4, /* bit2: R4 */
        GBC_LCD_GPIO_R5, /* bit3: R5 */
        GBC_LCD_GPIO_G2, /* bit4: G2 */
        GBC_LCD_GPIO_G3, /* bit5: G3 */
        GBC_LCD_GPIO_G4, /* bit6: G4 */
        GBC_LCD_GPIO_G5, /* bit7: G5 */
    };
    const int rgb332_data_gpio[8] = {
        GBC_LCD_GPIO_B4, /* bit0: B4 */
        GBC_LCD_GPIO_B5, /* bit1: B5 */
        GBC_LCD_GPIO_G3, /* bit2: G3 */
        GBC_LCD_GPIO_G4, /* bit3: G4 */
        GBC_LCD_GPIO_G5, /* bit4: G5 */
        GBC_LCD_GPIO_R3, /* bit5: R3 */
        GBC_LCD_GPIO_R4, /* bit6: R4 */
        GBC_LCD_GPIO_R5, /* bit7: R5 */
    };
    const int rgb664_data_gpio[16] = {
        GBC_LCD_GPIO_R0, /* bit0: R0 */
        GBC_LCD_GPIO_R1, /* bit1: R1 */
        GBC_LCD_GPIO_R2, /* bit2: R2 */
        GBC_LCD_GPIO_R3, /* bit3: R3 */
        GBC_LCD_GPIO_R4, /* bit4: R4 */
        GBC_LCD_GPIO_R5, /* bit5: R5 */
        GBC_LCD_GPIO_G0, /* bit6: G0 */
        GBC_LCD_GPIO_G1, /* bit7: G1 */
        GBC_LCD_GPIO_G2, /* bit8: G2 */
        GBC_LCD_GPIO_G3, /* bit9: G3 */
        GBC_LCD_GPIO_G4, /* bit10: G4 */
        GBC_LCD_GPIO_G5, /* bit11: G5 */
        GBC_LCD_GPIO_B2, /* bit12: B2 */
        GBC_LCD_GPIO_B3, /* bit13: B3 */
        GBC_LCD_GPIO_B4, /* bit14: B4 */
        GBC_LCD_GPIO_B5, /* bit15: B5 */
    };
    const int rgb565_data_gpio[16] = {
        GBC_LCD_GPIO_B1, /* bit0: B1 */
        GBC_LCD_GPIO_B2, /* bit1: B2 */
        GBC_LCD_GPIO_B3, /* bit2: B3 */
        GBC_LCD_GPIO_B4, /* bit3: B4 */
        GBC_LCD_GPIO_B5, /* bit4: B5 */
        GBC_LCD_GPIO_G0, /* bit5: G0 */
        GBC_LCD_GPIO_G1, /* bit6: G1 */
        GBC_LCD_GPIO_G2, /* bit7: G2 */
        GBC_LCD_GPIO_G3, /* bit8: G3 */
        GBC_LCD_GPIO_G4, /* bit9: G4 */
        GBC_LCD_GPIO_G5, /* bit10: G5 */
        GBC_LCD_GPIO_R1, /* bit11: R1 */
        GBC_LCD_GPIO_R2, /* bit12: R2 */
        GBC_LCD_GPIO_R3, /* bit13: R3 */
        GBC_LCD_GPIO_R4, /* bit14: R4 */
        GBC_LCD_GPIO_R5, /* bit15: R5 */
    };
                             ...
                             lcdcam_raw_result_t *result)
{
    ...
    esp_cam_ctlr_dvp_pin_config_t pin_config = {
        ...
        .data_io = {GBC_LCD_GPIO_R2, GBC_LCD_GPIO_R3, GBC_LCD_GPIO_R4, GBC_LCD_GPIO_R5,
                    GBC_LCD_GPIO_G2, GBC_LCD_GPIO_G3, GBC_LCD_GPIO_G4, GBC_LCD_GPIO_G5,
                    -1, -1, -1, -1, -1, -1, -1, -1},
        .pclk_io = GBC_LCD_GPIO_DCLK,
        .vsync_io = GBC_LCD_GPIO_SPS,
        .de_io = de_source == LCDCAM_RAW_DE_LP ? GBC_LCD_GPIO_LP : GBC_LCD_GPIO_SPL,
        .xclk_io = GPIO_NUM_NC,
    };
                                ...
                                lcdcam_raw_ctlr_stats_t *stats)
{
    ...
    stats->v_res = v_res;
    stats->bytes_per_sample = lcdcam_raw_bytes_per_sample(data_mode);
    stats->frame_bytes = h_res * v_res * stats->bytes_per_sample;
    stats->buffer_slots = LCDCAM_RAW_CTLR_BUFFER_SLOTS;
    stats->data_mode = data_mode;
    stats->last_esp_err = ESP_OK;
    stats->failure_stage = "none";

    uint8_t *buffers[LCDCAM_RAW_CTLR_BUFFER_SLOTS] = {0};
    int64_t *frame_done_us = NULL;
    SemaphoreHandle_t done_sem = NULL;
    ...
    size_t alignment_size = 0;
    int64_t started_us = esp_timer_get_time();
    lcdcam_raw_ctlr_context_t context = {
        .buffer_len = stats->frame_bytes,
        .requested_frames = frame_count,
        .last_finished_slot = UINT32_MAX,
    };
    ...
    stage = "allocate_frame_timing";
    frame_done_us = heap_caps_calloc(frame_count, sizeof(int64_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (frame_done_us == NULL) {
        err = ESP_ERR_NO_MEM;
        goto fail;
    }
    ...
    context.frame_done_us = frame_done_us;
    context.done_sem = done_sem;
    for (uint32_t i = 0; i < LCDCAM_RAW_CTLR_BUFFER_SLOTS; ++i) {
        context.buffers[i] = buffers[i];
    }

    stage = "esp_cam_new_dvp_ctlr";
    ...
    cam_ll_enable_vsync_generate_eof(hw, !byte_count_eof);
    if (byte_count_eof) {
        cam_ll_set_recv_data_bytelen(hw, stats->frame_bytes - 1U);
    }

    esp_cam_ctlr_evt_cbs_t cbs = {
        .on_get_new_trans = on_ctlr_get_new_trans,
        .on_trans_finished = on_ctlr_trans_finished,
    };
    ...
    context.started_us = esp_timer_get_time();
    started_us = context.started_us;
    ...
    uint32_t wait_ms = timeout_ms * frame_count;
    if (wait_ms < timeout_ms) {
        wait_ms = timeout_ms;
    }
    int64_t deadline_us = esp_timer_get_time() + (int64_t)wait_ms * 1000;
    while (context.completed_frames < frame_count && esp_timer_get_time() < deadline_us) {
        int64_t remaining_us = deadline_us - esp_timer_get_time();
        uint32_t remaining_ms = remaining_us > 0 ? (uint32_t)((remaining_us + 999) / 1000) : 1U;
        (void)xSemaphoreTake(done_sem, pdMS_TO_TICKS(MIN(remaining_ms, timeout_ms)));
    }

    if (context.completed_frames < frame_count) {
        err = ESP_ERR_TIMEOUT;
    ...
    stats->elapsed_us = esp_timer_get_time() - started_us;
    stats->completed_frames = MIN(context.completed_frames, frame_count);
    stats->dropped_frames = context.dropped_frames;
    stats->last_esp_err = err;

    int64_t total_interval_us = 0;
    for (uint32_t i = 0; i < stats->completed_frames; ++i) {
        int64_t interval_us = i == 0 ? frame_done_us[i] : frame_done_us[i] - frame_done_us[i - 1U];
        if (i == 0) {
            stats->first_frame_us = interval_us;
        } else {
            total_interval_us += interval_us;
        }
        if (interval_us > stats->max_frame_interval_us) {
            stats->max_frame_interval_us = interval_us;
        }
    }
    stats->avg_frame_interval_us = stats->completed_frames > 1U
                                       ? total_interval_us / (int64_t)(stats->completed_frames - 1U)
                                       : stats->first_frame_us;

    uint32_t checksum_slot = context.last_finished_slot < LCDCAM_RAW_CTLR_BUFFER_SLOTS ? context.last_finished_slot : 0U;
    (void)esp_cache_msync(buffers[checksum_slot], stats->frame_bytes, ESP_CACHE_MSYNC_FLAG_DIR_M2C);
    size_t sample_count = MIN((size_t)128, (size_t)stats->frame_bytes);
    for (size_t i = 0; i < sample_count; ++i) {
        stats->checksum = (stats->checksum * 33U) ^ buffers[checksum_slot][i];
    }
    ...
    heap_caps_free(frame_done_us);
    for (uint32_t i = 0; i < LCDCAM_RAW_CTLR_BUFFER_SLOTS; ++i) {
        heap_caps_free(buffers[i]);
    }
    return (err == ESP_OK && stats->completed_frames >= frame_count) ? ESP_OK : err;
                                ...
                                lcdcam_raw_ring_stats_t *stats)
{
    ...
    stats->bytes_per_sample = lcdcam_raw_bytes_per_sample(data_mode);
    stats->frame_bytes = h_res * v_res * stats->bytes_per_sample;
    stats->ring_slots = LCDCAM_RAW_RING_BUFFER_SLOTS;
    stats->data_mode = data_mode;
    stats->last_esp_err = ESP_OK;
    stats->failure_stage = "none";

    gdma_channel_handle_t dma_chan = NULL;
    uint8_t *buffers[LCDCAM_RAW_RING_BUFFER_SLOTS] = {0};
    lcdcam_raw_dma_desc_t *desc = NULL;
    int64_t *frame_done_us = NULL;
    SemaphoreHandle_t done_sem = NULL;
    bool dvp_initialized = false;
    size_t alignment_size = 0;
    size_t desc_count_per_slot = 0;
    size_t desc_stride_per_slot = 0;
    size_t desc_size_per_slot = 0;
    size_t desc_total_size = 0;
    size_t buffer_msync_len = 0;
    int64_t started_us = esp_timer_get_time();

    lcdcam_raw_ring_context_t context = {
        .done_sem = NULL,
        .hw = CAM_LL_GET_HW(0),
        .buffer_len = stats->frame_bytes,
        .requested_frames = frame_count,
    ...
    buffer_msync_len = align_up_size(stats->frame_bytes, alignment_size);
    ...
    stage = "allocate_descriptors";
    desc_count_per_slot = (stats->frame_bytes + LCDCAM_RAW_DMA_DESC_MAX_SIZE - 1U) / LCDCAM_RAW_DMA_DESC_MAX_SIZE;
    desc_size_per_slot = ((desc_count_per_slot * sizeof(lcdcam_raw_dma_desc_t)) + alignment_size - 1U) & ~(alignment_size - 1U);
    desc_stride_per_slot = desc_size_per_slot / sizeof(lcdcam_raw_dma_desc_t);
    desc_total_size = desc_size_per_slot * LCDCAM_RAW_RING_BUFFER_SLOTS;
    desc = heap_caps_aligned_calloc(alignment_size,
                                    1,
                                    desc_total_size,
                                    MALLOC_CAP_SPIRAM | MALLOC_CAP_DMA | MALLOC_CAP_8BIT);
    ...
    stats->descriptor_count_per_slot = desc_count_per_slot;
    ...
    context.dma_chan = dma_chan;
    context.done_sem = done_sem;
    context.desc = desc;
    context.buffers = buffers;
    context.desc_count_per_slot = desc_count_per_slot;
    context.desc_stride_per_slot = desc_stride_per_slot;
    context.desc_size_per_slot = desc_size_per_slot;
    context.frame_done_us = frame_done_us;

    gdma_rx_event_callbacks_t cbs = {
        .on_recv_eof = on_ring_recv_eof,
    };
    stage = "gdma_register_rx_event_callbacks";
    err = gdma_register_rx_event_callbacks(dma_chan, &cbs, &context);
    if (err != ESP_OK) {
    ...
    stage = "fill_ring_descriptors";
    fill_ring_descriptors(desc,
                          buffers,
                          LCDCAM_RAW_RING_BUFFER_SLOTS,
                          desc_count_per_slot,
                          desc_stride_per_slot,
                          stats->frame_bytes);
    err = esp_cache_msync(desc, desc_total_size, ESP_CACHE_MSYNC_FLAG_DIR_C2M | ESP_CACHE_MSYNC_FLAG_INVALIDATE);
    if (err != ESP_OK) {
        goto fail;
    }
    ...
    if (xSemaphoreTake(done_sem, pdMS_TO_TICKS(wait_ms)) != pdTRUE &&
        context.completed_frames < frame_count) {
        err = ESP_ERR_TIMEOUT;
        stage = "frame_done_timeout";
    } else {
        err = context.last_esp_err;
    ...
    stats->elapsed_us = esp_timer_get_time() - started_us;
    stats->completed_frames = MIN(context.completed_frames, frame_count);
    stats->ring_rearms = context.ring_rearms;
    stats->ring_rearm_failures = context.ring_rearm_failures;
    stats->unknown_eof_desc = context.unknown_eof_desc;
    stats->last_esp_err = err;
    int64_t total_interval_us = 0;
    for (uint32_t i = 0; i < stats->completed_frames; ++i) {
        int64_t interval_us = i == 0 ? frame_done_us[i] : frame_done_us[i] - frame_done_us[i - 1U];
    ...
    (void)esp_cache_msync(buffers[0], stats->frame_bytes, ESP_CACHE_MSYNC_FLAG_DIR_M2C);
    size_t sample_count = MIN((size_t)128, (size_t)stats->frame_bytes);
    for (size_t i = 0; i < sample_count; ++i) {
        stats->checksum = (stats->checksum * 33U) ^ buffers[0][i];
    }
                                       ...
                                       lcdcam_raw_ring_stats_t *stats)
{
    ...
    lcdcam_raw_dma_desc_t *desc = NULL;
    int64_t *frame_done_us = NULL;
    QueueHandle_t completed_slots = NULL;
    bool dvp_initialized = false;
    size_t alignment_size = 0;
    size_t desc_count_per_slot = 0;
    size_t desc_stride_per_slot = 0;
    ...
    lcdcam_raw_ring_stream_context_t context = {
        .hw = CAM_LL_GET_HW(0),
        .buffer_len = stats->frame_bytes,
        .requested_frames = frame_count,
    };
    ...
    stage = "xQueueCreate";
    completed_slots = xQueueCreate(LCDCAM_RAW_RING_BUFFER_SLOTS, sizeof(uint32_t));
    if (completed_slots == NULL) {
        err = ESP_ERR_NO_MEM;
        goto fail;
    }
    ...
    context.desc_count_per_slot = desc_count_per_slot;
    context.desc_stride_per_slot = desc_stride_per_slot;
    context.buffer_msync_len = buffer_msync_len;
    context.frame_done_us = frame_done_us;

    gdma_rx_event_callbacks_t cbs = {
        .on_recv_eof = on_ring_stream_recv_eof,
    };
    stage = "gdma_register_rx_event_callbacks";
    err = gdma_register_rx_event_callbacks(dma_chan, &cbs, &context);
    if (err != ESP_OK) {
    ...
    while (delivered_frames < frame_count && context.unknown_eof_desc == 0) {
        uint32_t slot = UINT32_MAX;
        if (xQueueReceive(completed_slots, &slot, pdMS_TO_TICKS(timeout_ms)) != pdTRUE) {
            err = ESP_ERR_TIMEOUT;
            stage = "completed_slot_timeout";
            break;
        }
        if (slot >= LCDCAM_RAW_RING_BUFFER_SLOTS) {
            err = ESP_ERR_INVALID_STATE;
            stage = "invalid_completed_slot";
            break;
        }

        lcdcam_raw_dma_desc_t *slot_base = &desc[slot * desc_stride_per_slot];
        (void)esp_cache_msync(buffers[slot], context.buffer_msync_len, ESP_CACHE_MSYNC_FLAG_DIR_M2C);

        lcdcam_raw_result_t frame_result = {
            .buffer = buffers[slot],
            .buffer_len = stats->frame_bytes,
            .received_size = stats->frame_bytes,
            .descriptor_count = desc_count_per_slot,
            .completed_descriptors = desc_count_per_slot,
            .timeout_ms = timeout_ms,
            .de_source = de_source,
            .vsync_invert = vsync_invert,
            .de_invert = de_invert,
            ...
            .data_mode = data_mode,
            .start_mode = start_mode,
            .start_trigger_seen = true,
            .eof_seen = true,
            .done_seen = true,
            .failure_stage = "none",
            .failure_err = ESP_OK,
        };
        copy_descriptor_report(&frame_result, slot_base, desc_count_per_slot);
        if (s_loop_summary_enabled) {
            summarize_buffer(&frame_result);
        }
        ...
        if (delivered_frames == 0) {
            capture_us = frame_done_us[0];
        } else {
            capture_us = frame_done_us[delivered_frames] - frame_done_us[delivered_frames - 1U];
        }
        if (frame_callback != NULL) {
            frame_callback(&frame_result, capture_us, callback_user_data);
        }

        rearm_descriptor_slot(slot_base, desc_count_per_slot, buffers[slot], stats->frame_bytes);
        restore_ring_descriptor_links(desc,
                                      LCDCAM_RAW_RING_BUFFER_SLOTS,
                                      desc_count_per_slot,
                                      desc_stride_per_slot);
        err = esp_cache_msync(desc,
                              desc_total_size,
                              ESP_CACHE_MSYNC_FLAG_DIR_C2M | ESP_CACHE_MSYNC_FLAG_INVALIDATE);
        if (err != ESP_OK) {
            stats->ring_rearm_failures++;
            stage = "esp_cache_msync_rearm_desc";
            break;
        }
        stats->ring_rearms++;
        delivered_frames++;
